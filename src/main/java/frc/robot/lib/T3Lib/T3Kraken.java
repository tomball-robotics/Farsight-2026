...
public class T3Kraken {
    ...
    */
    public static TalonFX createTalonFXPosition(int id, NeutralModeValue neutralMode, boolean inverted, double kP, double kI, double kD) {
        return createTalonFXPosition(id, null, neutralMode, inverted, DEFAULT_SUPPLY_CURRENT_LIMIT, DEFAULT_STATOR_CURRENT_LIMIT, kP, kI, kD, -1);
    }
    /**
     * 
     * Creates a TalonFX with an encoder
     */
     public static TalonFX createTalonFXPosition(int id, NeutralModeValue neutralMode, boolean inverted, double kP, double kI, double kD, int encoderID) {
        return createTalonFXPosition(id, null, neutralMode, inverted, DEFAULT_SUPPLY_CURRENT_LIMIT, DEFAULT_STATOR_CURRENT_LIMIT, kP, kI, kD, encoderID);
    }
    ...
    */
    public static TalonFX createTalonFXPosition(int id, NeutralModeValue neutralMode, boolean inverted, double supplyCurrentLimit, double statorCurrentLimit, double kP, double kI, double kD) {
        return createTalonFXPosition(id, null, neutralMode, inverted, supplyCurrentLimit, statorCurrentLimit, kP, kI, kD, -1);
    }
    ...
    */
    public static TalonFX createTalonFXPosition(int id, String canBus, NeutralModeValue neutralMode, boolean inverted, double supplyCurrentLimit, double statorCurrentLimit, double kP, double kI, double kD, int encoderID) {
        TalonFX motor = canBus != null ? new TalonFX(id, canBus) : new TalonFX(id);
        TalonFXConfiguration config = buildBaseConfig(neutralMode, inverted, supplyCurrentLimit, statorCurrentLimit);
        config.Slot0.kP = kP;
        config.Slot0.kI = kI;
